<template>
  <li class="relative">
    <div ref="triggerEl" class="flex items-center gap-1 transition select-none" :class="inPanel ? 'px-4 py-2 rounded cursor-pointer hover:bg-gray-100' : 'py-1'" @mouseenter="onTriggerEnter" @mouseleave="onTriggerLeave">
      <NuxtLink :to="item.link" class="flex-1" @click.prevent>
        {{ item.text }}
      </NuxtLink>

      <svg v-if="item.submenu" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mt-0.5 transition-transform" :class="{ 'rotate-90': open }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  <Teleport v-if="item.submenu" to="body">
    <ul v-show="open" ref="submenuEl" class="fixed z-[70] rounded-xl shadow-xl p-3 min-w-[12rem] max-w-xs max-h-[80vh] overflow-auto bg-white text-gray-700 transition-opacity duration-150" :style="submenuStyle" @mouseenter="onPanelEnter" @mouseleave="onPanelLeave">
      <DropdownRecursive v-for="sub in item.submenu" :key="sub.text" :item="sub" :in-panel="true"/>
    </ul>
  </Teleport>
</template>
...
function keepOpenChain () {
  cancelClose()
  parentKeepOpenChain && parentKeepOpenChain()
}

function scheduleCloseChain () {
  scheduleClose()
  parentScheduleCloseChain && parentScheduleCloseChain()
}
...
async function positionPanel () {
  ...
  if (props.inPanel) {
    ...
    submenuStyle.left = `${rect.left - 200}px` // مقدار موقت
  } else {
